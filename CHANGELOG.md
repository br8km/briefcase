...
### Changed
- `source.<name>.last_backup` and `remote.<name>.last_sync` now serialize in a concise local timestamp format such as `2026-04-22 14:37:05`
...
- Successful sync operations now persist `remote.<name>.last_sync` per successful remote, while `--dry-run` syncs leave timestamps unchanged
